...
export const ChatItem = ({
    ...
    useEffect(() => {
        const haddleKeyDown = (event: any) => {
            if (event.key === "Escape" || event.keyCod === 27){
                setIsEditing(false);
            }
        };

        window.addEventListener("keydown", haddleKeyDown);
        return () => window.removeEventListener("keydown", haddleKeyDown);
    },[])
                ...
                <div className="flex flex-col w-full">
                    <div className="flex items-center gap-x-2">
                        <div className="flex items-center">
                            <p  onClick={onMemberClick} className="font-semi-bold text-sm hover:underline">
                                {member.profile.name}  
                            </p>
                    ...
                    {!fileUrl && !isEditing && (
                        <p className={cn(
                            ...
                            deleted && "italic text-zinc-500 dark:text-zinc-400 text-xs mt-l"
                        )}>
                            {content}
                            {isUpdated && !deleted && (
                    ...
                    {canEditMessage && (
                        <ActionTooltip label="Edit">
                            <Edit
                                ...
                                hover:text-zinc-500 dark:text-zinc-300 transition"
                            />
                        </ActionTooltip>
                    )}
